...
const hasuraJwtSecretKey = process.env.HASURA_JWT_SECRET_KEY;
if (!hasuraJwtSecretKey) {
  throw Error("HASURA_JWT_SECRET_KEY must be defined in environment");
}

const hasuraJwtAudience = process.env.HASURA_JWT_AUDIENCE;
if (!hasuraJwtAudience) {
  throw Error("HASURA_JWT_AUDIENCE must be defined in environment");
}
...
async function setDatabaseConnectionUrl(): Promise<void> {
  ...
  const masterSecretArn =
    cdkOutputsData[`${appName}-HasuraStack`].HasuraDatabaseMasterSecretArn;
  ...
  const masterSecret = await secretsmanager
    .getSecretValue(getMasterSecretParams)
    .promise();
  ...
  const databaseUrlSecretArn =
    cdkOutputsData[`${appName}-HasuraStack`].HasuraDatabaseUrlSecretArn;
  ...
  const hasuraAdminSecretArn =
    cdkOutputsData[`${appName}-HasuraStack`].HasuraAdminSecretArn;
  ...
  const hasuraJwtSecretArn =
    cdkOutputsData[`${appName}-HasuraStack`].HasuraJwtSecretArn;
